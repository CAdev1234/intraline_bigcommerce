...
const SelectInput : FC<SelectInputProps> = ({option_li}) => {
    // let item_li = ['Dermal filler1', 'Dermal filler2', 'Dermal filler3']
    const [enableSelect, setEnableSelect] = useState(false)
    const [currentVal, setCurrentVal] = useState(option_li[0])
    
    return <div>
                <div className="h-11 w-106_5 border-none bg-c_F7F7F7 pl-5 py-2 relative cursor-pointer" onClick={() => {setEnableSelect(!enableSelect)}}>
                    <div>{currentVal}</div>
                                ...
                                {option_li.map((item, index) => {
                                    return <div key={`index_${index}`} onClick={() => {
                                        ...
                                        setEnableSelect(false)
                                                ...
                                                <div className=" h-11 flex items-center mx-5 border-b border-c_00080D cursor-pointer">{item}</div>
                                            </div>
                                })}
                            </div>
                        </div>
